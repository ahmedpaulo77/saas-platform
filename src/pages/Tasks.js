        ...
        <div className="table-container">
          ...
          {sortedTasks.length === 0 ? (
            <p style={{ textAlign: 'center', padding: '20px', color: '#999' }}>
              {searchTerm || filterStatus !== 'all' ? '❌ لا توجد نتائج مطابقة' : 'لا توجد مهام مسجلة حتى الآن'}
            </p>
          ) : (
            <table>
              <thead>
                  ...
                  <tr key={task.id}>
                    ...
                    <td>
                      <span className={`badge ${
                        task.priority === 'high' ? 'badge-expired' :
                        task.priority === 'medium' ? 'badge-pending' : 'badge-active'
                      }`}>
                    ...
                    <td>
                      <span className={`badge ${
                        task.status === 'completed' ? 'badge-paid' :
                        task.status === 'in-progress' ? 'badge-pending' : 'badge-expired'
                      }`}>
                        {task.status === 'completed' ? 'منجزة' :
                         task.status === 'in-progress' ? 'جاري التنفيذ' : 'قيد الانتظار'}
                      </span>
                    </td>
                    <td>{new Date(task.dueDate).toLocaleDateString('ar-EG')}</td>
        ...
        <div style={styles.modalOverlay} onClick={closeEditModal}>
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h3><i className="fas fa-edit"></i> تعديل المهمة</h3>
