...
export default function Projects() {
  ...
  const fetchProjects = useCallback(async () => {
    try {
      const querySnapshot = await getDocs(getScopedQuery('projects', userRole, userCompanyId));
      const projectsData = [];
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <div className="main-content">
        <h2 style={{ color: '#333', marginBottom: '20px' }}>
          <i className="fas fa-project-diagram" style={{ color: '#4f46e5' }}></i> إدارة المشاريع
        </h2>
        ...
        <div className="table-container">
          ...
          {filteredProjects.length === 0 ? (
            <p style={{ textAlign: 'center', padding: '20px', color: '#999' }}>
              {searchTerm ? '❌ لا توجد نتائج مطابقة للبحث' : 'لا توجد مشاريع مسجلة حتى الآن'}
            </p>
          ) : (
            <table>
              <thead>
              ...
              <tbody>
                {filteredProjects.map((project, index) => (
                  <tr key={project.id}>
                    ...
                    <td>{new Date(project.endDate).toLocaleDateString('ar-EG')}</td>
                    <td>{project.budget ? project.budget.toLocaleString() + ' ج.م' : '-'}</td>
                    <td>
                      <span className={`badge ${
                        project.status === 'completed' ? 'badge-paid' :
                        project.status === 'in-progress' ? 'badge-pending' :
                        project.status === 'on-hold' ? 'badge-expired' : 'badge-active'
                      }`}>
                        {project.status === 'completed' ? 'منجز' :
                         project.status === 'in-progress' ? 'جاري التنفيذ' :
                         project.status === 'on-hold' ? 'متوقف' : 'قيد الانتظار'}
                      </span>
                    </td>
                    <td>
                      <button
        ...
        <div style={styles.modalOverlay} onClick={closeEditModal}>
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h3><i className="fas fa-edit"></i> تعديل المشروع</h3>
