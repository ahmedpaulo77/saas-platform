...
export default function SuperAdminDashboard() {
  ...
  async function updateSubscription(companyId, newStatus) {
    if (!window.confirm(`تغيير الحالة إلى "${newStatus === 'active' ? 'نشط' : newStatus === 'trial' ? 'تجريبي' : 'منتهي'}"؟`)) return;
    try {
  ...
  const filtered = companies.filter(c => {
    ...
    const matchStatus = filterStatus === 'all' || c.subscription?.status === filterStatus;
      ...
      <div className="main-content">
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14 }}>{currentUser?.email}</p>
          </div>
          <span className="badge" style={{ background: 'rgba(245,158,11,0.2)', color: '#fcd34d', border: '1px solid rgba(245,158,11,0.3)', fontSize: 12 }}>
        ...
        <div className="filter-bar">
          <div className="search-wrapper" style={{ flex: 1 }}>
            ...
            <input type="text" placeholder="ابحث بالاسم أو البريد الإلكتروني..."
        ...
        <div className="table-container">
          <div className="table-header">
            <h3><i className="fas fa-list"></i> قائمة الشركات</h3>
                ...
                <tbody>
                  {filtered.map((company, i) => (
                    <tr key={company.id}>
                      ...
                      <td style={{ fontWeight: 700 }}>{company.name || 'غير محدد'}</td>
                      <td style={{ color: 'var(--gray-500)' }}>{company.email}</td>
                      <td>
                        <span className="badge badge-info">
                          {company.subscription?.plan === 'professional' ? '⚡ احترافي' :
                            company.subscription?.plan === 'enterprise' ? '🏢 مؤسسي' :
                              company.subscription?.plan === 'starter' ? '🚀 مبتدئ' :
                                company.plan === 'monthly' ? '📅 شهري' : company.plan === 'yearly' ? '📅 سنوي' : 'تجريبي'}
                        </span>
                      </td>
                      <td>
                      ...
                      <td>
                        <div className="table-actions">
                          ...
                          <button onClick={() => deleteCompany(company.id)} className="btn-danger btn-sm">
