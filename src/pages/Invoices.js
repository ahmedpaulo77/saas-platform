...
function OrderStatusBadge({ status, orderId, onStatusChange, isRestaurant }) {
  ...
  const nextStatus =
    currentIdx < ORDER_STATUSES.length - 2
...
export default function Invoices() {
  ...
  async function updateInvoice(e) {
    ...
    try {
      ...
      await updateDoc(doc(db, "invoices", editingInvoice.id), {
        ...
        deliveryAddress:
          editingInvoice.orderType === "delivery"
            ? editingInvoice.deliveryAddress || ""
            : "",
        deliveryPhone:
          editingInvoice.orderType === "delivery"
            ? editingInvoice.deliveryPhone || ""
            : "",
        deliveryFee:
          editingInvoice.orderType === "delivery"
            ? parseFloat(editingInvoice.deliveryFee) || 0
            : 0,
  ...
  async function recordPayment(e) {
    ...
    if (newPaid > total) {
      alert(t("in.payOver", { paid: newPaid, total }));
      return;
    }
    setPaying(true);
    try {
  ...
  function handleThermalPrint(invoice) {
    const clientName =
      clients.find((c) => c.id === invoice.clientId)?.name || "زبون";
    const orderTypeCfg = ORDER_TYPES.find((o) => o.value === invoice.orderType);
    const orderTypeLabel = orderTypeCfg
      ? orderTypeCfg.label
      : invoice.orderType || "";
...
<div class="center" style="font-size:11px;margin-top:6px;">شكراً لزيارتكم 🙏</div>
  ...
  const newOrdersCount = isRestaurant
    ? filteredInvoices.filter((i) => i.orderStatus === "new").length
    : 0;
  const preparingCount = isRestaurant
    ? filteredInvoices.filter((i) => i.orderStatus === "preparing").length
    : 0;
  const totalOverdue = filteredInvoices.reduce((sum, inv) => {
    if (inv.status === "overdue") {
      return (
        sum +
        ((parseFloat(inv.amount) || 0) - (parseFloat(inv.paidAmount) || 0))
      );
    }
    return sum;
  }, 0);
      ...
      <div className="main-content">
          ...
          <div>
            <h1>
              <i
                ...
                style={{
                  color: isRestaurant ? "#f59e0b" : "#f59e0b",
                  marginLeft: 10,
                }}
              ></i>
            ...
            <p className="subtitle">
              {isRestaurant ? "تسجيل ومتابعة طلبات المطعم" : t("in.subtitle")}
            </p>
          </div>
        </div>
          ...
          >
            <div className="stat-card amber">
              ...
              <div className="stat-label">
                {isRestaurant ? "إجمالي الطلبات" : t("in.statTotal")}
              </div>
            </div>
            {isRestaurant ? (
              <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div className="form-group">
                ...
                >
                  {showQuickAddClient
                    ? "✕ إلغاء"
                    : `+ إضافة ${isRestaurant ? "زبون" : entityLabel} جديد`}
                </button>
                {showQuickAddClient && (
                  <div
                    style={{
                  ...
                  >
                    ...
                    >
                      {addingClient
                        ? "جاري الحفظ..."
                        : `حفظ ${isRestaurant ? "الزبون" : entityLabel}`}
                    </button>
                  </div>
                )}
              </div>
              ...
              {newInvoice.products.length > 0 && (
                ...
                >
                  ...
                  >
                    {isRestaurant ? "أصناف الطلب" : t("in.selectedProducts")} (
                    {newInvoice.products.length})
                  </h4>
                          ...
                          >
                            ...
                            <button
                              ...
                              onClick={() =>
                                setNewInvoice({
                                  ...newInvoice,
                                  products: newInvoice.products.filter(
                                    (_, i) => i !== idx,
                                  ),
                                })
                              }
                  ...
                  >
                    <span>الإجمالي:</span>
                    <span>
                      {getTotalAmount.toLocaleString()} {t("currency")}
                    </span>
                  </div>
                </div>
              )}
              ...
              {isRestaurant && (
                <>
                  ...
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    ...
                    <input
                      type="text"
                      placeholder="مثال: بدون بصل، صوص على الجانب..."
                      ...
                      {ORDER_STATUSES.filter(
                        ...
                        <button
                          ...
                          style={{
                            ...
                            background:
                              (newInvoice.orderStatus || "new") === s.value
                                ? s.bg
                                : "white",
                            color:
                              (newInvoice.orderStatus || "new") === s.value
              ...
              >
                {submitting ? (
                  ...
                  <>
                    ...
                    {isRestaurant ? "تسجيل الطلب" : t("in.add")}
                  </>
                )}
              </button>
            </div>
        ...
        <div className="filter-bar">
          <div className="search-wrapper" style={{ flex: 1 }}>
            ...
            <input
              type="text"
              placeholder={isRestaurant ? "ابحث عن طلب..." : t("in.search")}
              value={searchTerm}
        ...
        <div className="table-container">
          <div className="table-header">
            <h3>
              ...
              {isRestaurant ? "قائمة الطلبات" : t("in.list")}
            </h3>
            <span className="table-count">
              {filteredInvoices.length}{" "}
              {isRestaurant ? "طلب" : t("in.invoices")}
            </span>
          </div>
                  ...
                  <tbody>
                    {pageItems.map((inv, i) => {
                      ...
                      const totalWithFee =
                        ...
                        (parseFloat(inv.deliveryFee) || 0);
                      const orderTypeCfg = ORDER_TYPES.find(
                        (o) => o.value === inv.orderType,
                      );
                      ...
                      return (
                        <tr key={inv.id}>
                          ...
                          <td style={{ fontWeight: 600 }}>
                            ...
                            {isRestaurant && inv.customerNote && (
                              <div
                                ...
                                title={inv.customerNote}
                              >
                                📝 {inv.customerNote.slice(0, 25)}
                                {inv.customerNote.length > 25 ? "..." : ""}
                              </div>
                            )}
                          </td>
                          {isRestaurant && (
                            <td>
                              ...
                              >
                                <span style={{ fontWeight: 600 }}>
                                  {orderTypeCfg?.label || inv.orderType || "—"}
                                </span>
                                {inv.orderType === "delivery" &&
                                  inv.deliveryAddress && (
                                    <span
                                      style={{ fontSize: 11, color: "#64748b" }}
                                      title={inv.deliveryAddress}
                                    >
                                      📍 {inv.deliveryAddress.slice(0, 20)}
                                      {inv.deliveryAddress.length > 20
                                        ? "..."
                                        : ""}
                                    </span>
                                  )}
                          ...
                          {hasInventory && (
                            <td style={{ fontSize: 13 }}>{productStr}</td>
                          )}
                          ...
                          <td>
                            <div className="table-actions">
                              ...
                              {isRestaurant && (
                                <button
                                  onClick={() => handleThermalPrint(inv)}
                                  className="btn-primary btn-sm"
                                  title="طباعة فاتورة"
                                >
            ...
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header">
                <h3>
                  ...
                  {isRestaurant ? "تعديل الطلب" : t("common.edit")}
                </h3>
                <button
                ...
                >
                  <div className="form-group">
                    ...
                    <AutocompleteInput
                      ...
                      placeholder={
                        isRestaurant ? "اختر الزبون" : chooseEntityPlaceholder
                      }
                      required
                    />
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      readOnly={editingInvoice.products?.length > 0}
                      required
                    />
                  </div>
                  ...
                  {isRestaurant && (
                    <>
                          ...
                          {ORDER_STATUSES.map((s) => (
                            <button
                              ...
                              style={{
                                ...
                                background:
                                  (editingInvoice.orderStatus || "new") ===
                                  s.value
                                    ? s.bg
                                    : "white",
                                color:
                          ...
                          {ORDER_TYPES.map((ot) => (
                            <button
                              ...
                              style={{
                                ...
                                borderRadius: 10,
                                background:
                                  (editingInvoice.orderType || "takeaway") ===
                                  ot.value
                                    ? "#fffbeb"
                                    : "white",
                                color:
                      ...
                      {(editingInvoice.orderType || "takeaway") ===
                        ...
                        <>
                          <div className="form-group">
                            ...
                            <input
                              type="text"
                              value={editingInvoice.deliveryAddress || ""}
                              onChange={(e) =>
                                setEditingInvoice({
                                  ...editingInvoice,
                                  deliveryAddress: e.target.value,
                          ...
                          <div className="form-group">
                            ...
                            <input
                              type="tel"
                              value={editingInvoice.deliveryPhone || ""}
                              onChange={(e) =>
                                setEditingInvoice({
                                  ...editingInvoice,
                                  deliveryPhone: e.target.value,
                          ...
                          <div className="form-group">
                            ...
                            <input
                              ...
                              step="0.5"
                              min="0"
                              value={editingInvoice.deliveryFee || ""}
                              onChange={(e) =>
                                setEditingInvoice({
                                  ...editingInvoice,
                                  deliveryFee: e.target.value,
                      ...
                      <div className="form-group">
                        ...
                        <input
                          type="text"
                          value={editingInvoice.customerNote || ""}
                          onChange={(e) =>
                            setEditingInvoice({
                              ...editingInvoice,
                              customerNote: e.target.value,
                    ...
                    >
                      ...
                      >
                        ...
                        <span style={{ color: "#6366f1" }}>
                          ({editingInvoice.products.length})
                        </span>
                      </h4>
                      <div
                        style={{
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setEditingInvoice({
                                    ...editingInvoice,
                                    products: editingInvoice.products.filter(
                                      (_, i) => i !== idx,
                                    ),
                                  })
                                }
