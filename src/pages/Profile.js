...
export default function Profile() {
  ...
  async function handlePasswordChange(e) {
    ...
    if (newPassword.length < 6) { setError('يجب أن تكون كلمة المرور 6 أحرف على الأقل'); return; }
  ...
  const joinDate = currentUser?.metadata?.creationTime
    ? new Date(currentUser.metadata.creationTime).toLocaleDateString('ar-EG', { year: 'numeric', month: 'long', day: 'numeric' })
    : 'غير محدد';
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <div className="main-content">
        ...
        <div className="header">
          <div>
            <h1><i className="fas fa-user-circle" style={{ color: '#6366f1', marginLeft: 10 }}></i>الملف الشخصي</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(320px,1fr))', gap: 24, alignItems: 'start' }}>
          ...
          <div className="card" style={{ textAlign: 'center', padding: '36px 28px' }}>
            ...
            <div style={{ display: 'inline-flex', gap: 8, marginBottom: 24 }}>
              <span className={`badge ${userRole === 'super_admin' ? 'badge-purple' : 'badge-info'}`}>
                <i className={`fas ${userRole === 'super_admin' ? 'fa-crown' : 'fa-user'}`}></i>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, textAlign: 'right' }}>
              {[
                { icon: 'fas fa-envelope', label: 'البريد الإلكتروني', value: currentUser?.email },
                { icon: 'fas fa-calendar-alt', label: 'تاريخ التسجيل', value: joinDate },
                { icon: 'fas fa-shield-alt', label: 'التحقق من البريد', value: currentUser?.emailVerified ? 'مؤكد ✓' : 'غير مؤكد' },
              ].map(item => (
                <div key={item.label} style={{
                  ...
                  <i className={item.icon} style={{ color: 'var(--primary)', width: 16, textAlign: 'center' }}></i>
                  <div>
                    <div style={{ fontSize: 11, color: 'var(--gray-400)', fontWeight: 600 }}>{item.label}</div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--gray-700)' }}>{item.value}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="card">
            ...
            <form onSubmit={handlePasswordChange}>
              ...
              {newPassword.length > 0 && (
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 12, color: 'var(--gray-500)', marginBottom: 6 }}>قوة كلمة المرور:</div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--gray-400)', marginTop: 4 }}>
                    {newPassword.length < 3 ? 'ضعيفة جداً' :
                      newPassword.length < 6 ? 'ضعيفة' :
                        newPassword.length < 9 ? 'متوسطة' : 'قوية'}
                  </div>
                </div>
              )}
            ...
            <div style={{ marginTop: 16, padding: 12, background: 'var(--info-bg)', borderRadius: 8 }}>
