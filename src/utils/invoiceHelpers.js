...
export function buildThermalPrintHTML({ invoice, clientName, products, isTrader, getProductUnit, isKgUnit, t }) {
  ...
  const itemsRows = (invoice.products || [])
    ...
    .join("");
  const deliveryInfo = invoice.orderType === "delivery" ? `<div style="margin:6px 0;font-size:12px;"><strong>📍 العنوان:</strong> ${invoice.deliveryAddress || "—"}<br/>${invoice.deliveryPhone ? `<strong>📞 هاتف:</strong> ${invoice.deliveryPhone}` : ""}${invoice.deliveryFee > 0 ? `<br/><strong>🛵 رسوم التوصيل:</strong> ${invoice.deliveryFee} ج.م` : ""}</div>` : "";
